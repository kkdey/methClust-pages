...
```{r}
latlong_chars <- readRDS("../data/LatLongCells.rds")
latlong <- cbind.data.frame(
...
```{r}
idx1 <- which(latlong[,2] > 5 & latlong[,2] < 50)
idx2 <- which(latlong[,1] > 50 & latlong[,1] < 120)
...
rownames(birds_pa_data_3) <- paste0(latlong2[,1], "_", latlong2[,2])
save(birds_pa_data_3, file = "../../ecostructure/data/indian_birds_pa.rda")
```
